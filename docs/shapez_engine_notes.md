...
## Storage & Mod Loader Scoping (`src/store.js`, `src/storageSelection.js`)
- `storage` is a closure-local variable inside `modloader.js`'s `initMods()` — it is **not** exposed as `mod.app.storage` or `mod.storage`. Reconstruct the storage backend (`StorageImplElectron` / `StorageImplBrowserIndexedDB`) from `this.app` inside `mod.init()` instead of assuming it's attached somewhere. This isn't just scoping — `application.js`'s `boot()` calls `MODS.initMods()` before `this.storage` is even nulled out, let alone assigned; the real `app.storage` isn't set until `platformWrapper.initialize()` runs inside `PreloadState.startLoading()`, well after mod init has already finished. There's no "wait for the real storage" option at mod-init time — reconstructing your own instance is the only thing that works that early.
