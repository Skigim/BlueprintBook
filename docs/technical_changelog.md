...
### 8. Tombstones Key on Value, Not Name (`src/store.js`, `src/metadata.js`)
- **Root Cause**: `remove()` recorded both a blueprint's `value` and its `name` into `deletedValues` / `deletedNames`, and `_mergeBlueprintIfNew` rejected a migration candidate when **either** matched. A name is a reusable label, not an identity — so deleting any blueprint named e.g. `Paint Mixer` blacklisted that name permanently, and any later blueprint reusing it was silently discarded by the next version bump's migration scan. Confirmed against a real 1.0.3 save: 4 of 6 blueprints were dropped on the 1.0.3 → 1.0.4 migration, all 4 with their name in `deletedNames` and none with their value in `deletedValues`.
- **Fix**: `deletedNames` is removed outright — from the merge check, the per-file tombstone accumulation in `migrateLegacySettings`, `remove()`, the `init()` guards, and `METADATA.settings`. `_mergeBlueprintIfNew(bp, currentBlueprints, existingValues, deletedValues)` now keys on `value` alone, and ignores entries with no `value` (no content to restore, and nothing to identify them by). Any `deletedNames` key left in an existing save is simply never read.
- **Same flaw, second site**: the `existingNames` de-duplication set went with it. It would have collapsed two genuinely distinct blueprints that happened to share a name into a single entry during migration — the same false-identity assumption, one step further along.
- **Also removed**: the unused `scannedLegacyValues` / `scannedLegacyNames` sets, left behind by tombstone-pruning logic that no longer exists.
- **Verification**: beyond unit tests, the real 1.0.3 save was replayed through `BlueprintStore.init` against a fresh `METADATA.settings` object simulating a 1.0.4 install — all 6 blueprints and the `Industries` tag survive.
